...
window.elements.app = Polymer({
  ...
  syncFromService: function() {
    return this.getGoogleTracks().then((function(_this) {
      return function(googleTracks) {
        var i, key, len, results, track;
        console.log(googleTracks);
        results = [];
        for (key = i = 0, len = googleTracks.length; i < len; key = ++i) {
          track = googleTracks[key];
          results.push((function(track) {
            if (key < 60) {
  ...
  displayPrelim: function(ctx, next) {
    var login;
    console.log("prelim");
    ...
    if (this.menu == null) {
      ...
      this.menu = new daElements.daMenu();
      this.$.menuDrawer.appendChild(this.menu);
    }
    this.menu.closeSlide();
    while (this.$.display.firstChild != null) {
      this.$.display.removeChild(this.$.display.firstChild);
    }
    if (ctx.path === "/test" || ctx.path === "/logout") {
      console.log("found test or logout path");
      next();
    }
    if (ctx.path === "/login" || (Cookies.get('token') == null) || (Cookies.get('user') == null)) {
      this.hideSpinner();
      login = new daElements.daLogin();
      this.$.display.appendChild(login);
    } else {
      if ((this.cases == null) || (this.events == null) || (this.todos == null)) {
        console.log("loading");
  ...
  displayLogout: function(ctx, next) {},
  displayHome: function(ctx, next) {
    if (this.home == null) {
      this.home = new daElements.daHome();
    }
    return this.$.display.appendChild(this.home);
  },
  displayCases: function(ctx, next) {
    console.log("displaying cases");
    if (this.caseList == null) {
      this.caseList = new daElements.daCaseList();
    }
    return this.$.display.appendChild(this.caseList);
  },
  displayToDoList: function() {
    if (this.todoList == null) {
      this.todoList = new daElements.daTodoList();
    }
    return this.$.display.appendChild(this.todoList);
  },
  displayNotificationsList: function() {
    if (this.notificationsElem == null) {
      this.notificationsElem = new daElements.daNotificationList();
    }
    this.notificationsElem.user = this.user.name;
    return this.$.display.appendChild(this.notificationsElem);
  },
  displayNineOneOneList: function() {
    console.log("yes");
    if (this.nooElem == null) {
      this.nooElem = new daElements.da911List();
    }
    return this.$.display.appendChild(this.nooElem);
  },
  displayCrbList: function() {
    if (this.crbElem == null) {
      this.crbElem = new daElements.daCrbList();
    }
    return this.$.display.appendChild(this.crbElem);
  },
  displayTest: function() {
    console.log("test path");
    if (this.testElem == null) {
      this.testElem = new daElements.daTestSuite();
    }
    return this.testElem.testCaseConverter();
  },
...
},{"../../node_modules/jquery":2,"../../node_modules/js-cookie":3,"../../node_modules/underscore":4}],2:[function(require,module,exports){
